...
poi.VC <- function(fname, cell, on.reward = TRUE, match.level = TRUE, AB.eqlevel = FALSE, outfile = "", start = 0, end = 600, go.by.soff = TRUE){
  ...
  for(fr in alt.freq){
    for(po in alt.pos){
      try({lbf <- round(esti.Poi(trials, spiketimes, c(fr, 742), c(po, -144/po), on.reward, start, end, match.level, AB.eqlevel, go.by.soff), 4);
...
poi.JA <- function(fname, on.reward = TRUE, match.level = FALSE, AB.eqlevel = FALSE, outfile = "", start = 0, end = 600){
  ...
  for(fr in alt.freq){
    for(po in alt.pos){
      try({lbf <- round(esti.Poi(trials, spiketimes, c(fr, 742), c(po, -144/po), on.reward, start, end, match.level, AB.eqlevel, FALSE), 4);
