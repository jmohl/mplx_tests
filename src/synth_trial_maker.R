##functions for generating the synthetic data used for testing
#
#--------------------
#Jeff mohl
#last edit 6/7/18
...
#----------------------
#make_trial_data: generates the trial meta data, the matches the format used for the IC dataset
#make_synth_data: calls spike generator and generates spike times according to different hypotheses
...
make_synth_data <- function(trial.nums, start.time = -200, end.time = 800, bw = 25,lambdaA=rep(0,(end.time-start.time)/bw),lambdaB=rep(0,(end.time-start.time)/bw),control.type=NULL, ...){
   ...
   if(!is.null(control.type)){
      for (ABindex in trial.nums) {
         ...
         #else if (control.type == "smoo_mplxd") added.spikes <- gen_smooth_mplxd(lambdaA = lambdaA, lambdaB=lambdaB, num.ctrials=1,start.time=start.time, end.time=end.time, bw=bw)
         else added.spikes <- 0
