...
get.bayes.factors <- function(xA, xB, xAB, labels = c("A", "B", "AB")){
  nA <- length(xA)
  nB <- length(xB)
  nAB <- length(xAB)
  if(nA == 0 | nB == 0 | nAB ==0) stop("not enough data in single sound")
  ...
  log.marg.dom <- max(log.marg.dom.1,log.marg.dom.2)
   bfs <- exp(c(log.marg.mix, log.marg.ave, log.marg.out, log.marg.dom) - log.marg.ind)
	title(main = paste("Psep:", round(100*plogis(lbf.pure)), "%, mx/av:", signif(bfs[1]/bfs[2],1), ", chiSq:", signif(min(pvls[1:2]),1), sep = ""), font.main=2, cex.main = 0.9)
